  ...
  return (
    <div className={classes.container}>
      <div className={classes.content}>
        <Title title={isEditMode ? "Editer Le Plat" : "Ajouter Un Plat"} />
        <form
          className={classes.form}
          onSubmit={handleSubmit(submit)}
          noValidate
        >
          <InputContainer label="Sélectionner Une Image">
            <input type="file" onChange={upload} accept="image/jpg" />
          </InputContainer>
          ...
          <Input
            type="text"
            label="Temps De Cuisinne"
            {...register("cookTime", { required: true })}
            error={errors.cookTime}
          />

          <Button type="submit" text={isEditMode ? "Mettre à jour" : "Créer"} />
        </form>
      </div>
    </div>
  );
